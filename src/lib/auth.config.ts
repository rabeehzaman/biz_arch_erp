  ...
  callbacks: {
    ...
    async jwt({ token, user, trigger, session: sessionUpdate }) {
      if (user) {
        ...
        token.isScannerEnabled = (user as { isScannerEnabled?: boolean }).isScannerEnabled ?? false;
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        (session.user as { isScannerEnabled?: boolean }).isScannerEnabled = token.isScannerEnabled as boolean;
