...
export async function GET(request: NextRequest) {
  try {
    ...
    const [organization, data] = await Promise.all([
      ...
      getProfitLossData(organizationId, fromDate, toDate, branchId),
    ]);
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="profit-loss-${fromDate}-to-${toDate}.pdf"`,
      },
    });
  } catch (error) {
    console.error("Failed to generate profit & loss PDF:", error);
    return NextResponse.json(
      { error: "Failed to generate profit & loss PDF" },
      { status: 500 }
    );
  }
}
