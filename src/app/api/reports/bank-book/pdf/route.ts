  ...
  try {
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="bank-book-${fromDate}-to-${toDate}.pdf"`,
      },
    });
  } catch (error) {
    console.error("Failed to generate bank book PDF:", error);
    return NextResponse.json(
      { error: "Failed to generate bank book PDF" },
      { status: 500 }
    );
  }
}
