...
import { CashFlowPDF } from "@/components/pdf/cash-flow-pdf";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [organization, data] = await Promise.all([
      ...
      getCashFlowData(organizationId, fromDate, toDate, branchId),
    ]);
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="cash-flow-${fromDate}-to-${toDate}.pdf"`,
      },
    });
  } catch (error) {
