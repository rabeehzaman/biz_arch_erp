...
export async function POST(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Create journal entry (Credit Cash/Bank, Debit equity/owner)
      ...
      if (account.accountId && equityAccount) {
        await createAutoJournalEntry(tx, organizationId, {
          ...
          lines: [
            { accountId: equityAccount.id, debit: Number(amount), credit: 0 },
            { accountId: account.accountId, debit: 0, credit: Number(amount) },
          ],
        });
      }
