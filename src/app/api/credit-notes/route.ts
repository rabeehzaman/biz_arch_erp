...
export async function POST(request: NextRequest) {
  try {
    ...
    // ZATCA Phase 2: Submit credit note to ZATCA
    ...
    if (saudiEnabled && result) {
      ...
      if (org?.zatcaPhase2Active) {
        try {
          ...
          console.error("ZATCA Phase 2 credit note submission error:", error);
          zatcaWarning = "Credit note created but ZATCA submission failed. Retry from ZATCA dashboard.";
        }
      }
    }
