...
export default function SettingsPage() {
  ...
  const sessionRestaurant = (session?.user as { isRestaurantModuleEnabled?: boolean })?.isRestaurantModuleEnabled ?? false;
  const [isRestaurantEnabled, setIsRestaurantEnabled] = useState(sessionRestaurant);
  const [activeTab, setActiveTab] = useState<SettingsTab>("units");

  // Fetch live org flags so we don't need re-login after admin enables them
  useEffect(() => {
    setIsRestaurantEnabled(sessionRestaurant);
    fetch("/api/settings")
      .then((r) => r.json())
      .then((data) => {
        if (typeof data.isRestaurantModuleEnabled === "boolean") setIsRestaurantEnabled(data.isRestaurantModuleEnabled);
      })
      .catch(() => {});
  }, [sessionRestaurant]);
