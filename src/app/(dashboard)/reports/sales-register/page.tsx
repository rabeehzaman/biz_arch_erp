...
export default function SalesRegisterPage() {
  ...
  const handleExportCsv = () => {
    ...
    downloadCsv([header, ...rows, totalsRow], `sales-register-${fromDate}-to-${toDate}.csv`);
  };
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      if (branchParam) params.set("branchId", branchParam);
      const response = await fetch(`/api/reports/sales-register/pdf?${params}`);
      if (!response.ok) throw new Error("Failed to generate PDF");
      const blob = await response.blob();
      await downloadBlob(blob, `sales-register-${fromDate}-to-${toDate}.pdf`);
    } catch {
  ...
  return (
    <ReportPageLayout
      titleKey="reports.salesRegister"
      descriptionKey="reports.salesRegisterDesc"
      isLoading={isLoading}
      actions={
        <ReportExportButton
