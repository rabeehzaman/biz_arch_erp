  ...
  return (
    <PageAnimation>
      <div className="space-y-6">
        ...
        {debitNote.zatcaSubmissions && debitNote.zatcaSubmissions.length > 0 && (() => {
          const sub = debitNote.zatcaSubmissions[0];
                  ...
                  <div className="flex items-end">
                    {(sub.status === "FAILED" || sub.status === "REJECTED") && (
                      <Button variant="outline" size="sm" onClick={async () => {
                        try {
                          const res = await fetch(`/api/zatca/submit/${debitNote.id}?type=debit-note`, { method: "POST" });
