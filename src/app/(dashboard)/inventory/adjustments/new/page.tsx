...
import {
  Trash2, ArrowLeft, Save, TrendingUp, TrendingDown, Package,
} from "lucide-react";
...
export default function NewStockTakePage() {
  ...
  // Fetch products with stock when warehouse changes
  useEffect(() => {
    if (!warehouseId) {
      setProducts([]);
      return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setIsDirty(false);
        toast.success(t("inventory.stockTakeCreated"));
    ...
    <PageAnimation>
      <div className="space-y-6">
        ...
        <div className="flex items-start gap-3 sm:items-center sm:gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">{t("inventory.newStockTake")}</h2>
