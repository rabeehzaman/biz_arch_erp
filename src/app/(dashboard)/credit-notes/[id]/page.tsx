      ...
      <div className="space-y-4 sm:space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-row sm:flex-wrap">
            ...
            <Button variant="outline" size="sm" onClick={() => router.push(`/credit-notes/new?duplicate=${creditNote.id}`)} className="col-span-1 h-9 w-full sm:h-10 sm:w-auto">
        ...
        {creditNote.zatcaSubmissions && creditNote.zatcaSubmissions.length > 0 && (() => {
          const sub = creditNote.zatcaSubmissions[0];
                  ...
                  <div className="flex items-end">
                    {(sub.status === "FAILED" || sub.status === "REJECTED") && (
                      <Button variant="outline" size="sm" onClick={async () => {
                        try {
                          const res = await fetch(`/api/zatca/submit/${creditNote.id}?type=credit-note`, { method: "POST" });
