...
export default function CreateSupplierPaymentPage() {
  ...
  const fetchInvoices = async () => {
    const response = await fetch("/api/purchase-invoices?limit=500");
  ...
  const supplierInvoices = useMemo(
    () => allInvoices.filter((inv) => inv.supplierId === formData.supplierId),
    [allInvoices, formData.supplierId]
  );

  const computedAllocations = useMemo(() => {
    const totalPayment = (parseFloat(formData.amount) || 0) + (parseFloat(formData.discountGiven) || 0);
    if (totalPayment <= 0 || supplierInvoices.length === 0) return {};
    ...
    return result;
  }, [formData.amount, formData.discountGiven, supplierInvoices, manualAllocations]);
    ...
    <PageAnimation>
      <div className="space-y-6 pb-32 sm:pb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">{t("payments.recordSupplierPayment")}</h2>
            <p className="text-slate-500">{t("payments.recordSupplierPaymentDesc")}</p>
          </div>
        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Combobox
                    ...
                    onValueChange={(value) => {
                      ...
                      if (value) setFormErrors((prev) => ({ ...prev, supplierId: "" }));
                    }}
                    getId={(s) => s.id}
                    getLabel={(s) => s.name}
                    filterFn={(s, query) => s.name.toLowerCase().includes(query)}
                    renderItem={(s) => (
                      <div className="flex justify-between w-full">
                        <span>{s.name}</span>
                        <span className="text-slate-500 text-xs">
                          {t("statement.payable")}: {fmt(Number(s.balance))}
                        </span>
                      </div>
                    )}
                  ...
                  {selectedSupplier && (
                    <p className="text-sm text-slate-500">
                      {t("statement.payable")}: <span className="font-medium">{fmt(Number(selectedSupplier.balance))}</span>
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    <Label>{t("payments.discountGiven")}</Label>
                    <Input
                      type="number"
                      step="0.001"
                      value={formData.discountGiven}
                      onChange={(e) => setFormData((prev) => ({ ...prev, discountGiven: e.target.value }))}
                      placeholder="0.00"
                    />
                  </div>
                </div>
                ...
                {isAdjustment && (
                  <div className="grid gap-2">
                    <Label>{t("common.adjustmentAccount")} *</Label>
                    <Combobox
                      items={accounts}
                  ...
                  {supplierInvoices.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      ...
                      <p className="mt-3 text-sm text-slate-500">
                        {t("payments.noUnpaidInvoices") || "No unpaid invoices for this supplier"}
                      </p>
                    </div>
                  ) : (
                    <>
                                ...
                                <TableRow key={inv.id}>
                                  ...
                                  <TableCell className="text-right">
                                    <Input
                                      ...
                                      placeholder="0.00"
                                      className={`w-[140px] text-right ml-auto ${allocated > 0 ? "border-green-300" : ""}`}
                                    />
                                  </TableCell>
                                </TableRow>
                              );
                          ...
                          return (
                            <div key={inv.id} className="rounded-xl border border-slate-200 bg-white p-4">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <p className="font-medium text-slate-900">{inv.purchaseInvoiceNumber}</p>
            ...
            <div className="sticky top-6">
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  {(parseFloat(formData.discountGiven) || 0) > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-500">{t("payments.discountGiven")}</span>
                      <span className="font-medium">{fmt(parseFloat(formData.discountGiven) || 0)}</span>
                    </div>
                  )}
        ...
        <StickyBottomBar
          topContent={
            <div className="flex justify-between text-sm">
              ...
              {supplierInvoices.length > 0 && totalAllocated > 0 && (
