...
export function CustomerCommentsTab({ customerId }: { customerId: string }) {
  const { t } = useLanguage();
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      setContent("");
      toast.success(t("customerDetail.commentPosted"));
    } catch (error) {
  ...
  const handleDelete = async (commentId: string) => {
    ...
    try {
      ...
      toast.success(t("customerDetail.commentDeleted"));
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="pt-6">
          <textarea
            ...
            rows={3}
            placeholder={t("customerDetail.writeComment")}
            value={content}
          ...
          <div className="mt-3 flex items-center justify-between">
            <p className="text-xs text-slate-400">
              {t("customerDetail.commentShortcut")}
            </p>
      ...
      {comments.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <h3 className="mt-4 text-lg font-semibold text-slate-600">{t("customerDetail.noComments")}</h3>
          <p className="text-sm text-slate-400">{t("customerDetail.noCommentsDesc")}</p>
        </div>
      ) : (
        <div className="space-y-4">
