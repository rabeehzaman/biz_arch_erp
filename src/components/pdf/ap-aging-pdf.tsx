...
import type { APAgingData } from "@/lib/reports/ap-aging";

interface APAgingPDFProps {
...
const labels = {
  en: {
    ...
    supplier: "Supplier",
    totals: "Totals",
    noData: "No outstanding payables.",
    page: "Page",
...
export function APAgingPDF({ organization, data, asOfDate, lang }: APAgingPDFProps) {
  const isRTL = lang === "ar";
  const l = labels[lang];
        ...
        {data.suppliers.length === 0 ? (
          ...
          <View style={styles.table}>
            <View style={styles.tableHeader}>
              <Text style={[styles.tableHeaderText, styles.colSupplier]}>{l.supplier}</Text>
            ...
            {data.suppliers.map((row, i) => (
              <View key={row.supplierId} style={i % 2 === 0 ? styles.tableRow : styles.tableRowAlt} wrap={false}>
                <Text style={[styles.cellText, styles.colSupplier]}>{row.supplierName}</Text>
            ...
            <View style={styles.tableTotalsRow} wrap={false}>
              <Text style={[styles.cellTextBold, styles.colSupplier]}>{l.totals}</Text>
