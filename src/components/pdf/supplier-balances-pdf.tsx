...
import type { SupplierBalancesData } from "@/lib/reports/supplier-balances";

interface SupplierBalancesPDFProps {
  organization: { name: string; arabicName?: string | null; brandColor?: string | null; currency?: string | null };
  data: SupplierBalancesData;
  lang: "en" | "ar";
}
...
export function SupplierBalancesPDF({ organization, data, lang }: SupplierBalancesPDFProps) {
  const isRTL = lang === "ar";
  const l = labels[lang];
      ...
      <Page size="A4" style={styles.page} wrap>
        ...
        <View style={styles.heroStats}>
          <View style={styles.heroCard}>
            <Text style={styles.heroLabel}>{l.totalSuppliers}</Text>
            <Text style={styles.heroValue}>{data.summary.totalSuppliers}</Text>
          </View>
          <View style={styles.heroCard}>
            <Text style={styles.heroLabel}>{l.active}</Text>
            <Text style={styles.heroValueGreen}>{data.summary.activeSuppliers}</Text>
          </View>
          <View style={styles.heroCard}>
            <Text style={styles.heroLabel}>{l.totalPayable}</Text>
            <Text style={styles.heroValueRed}>{formatAmount(data.summary.totalPayable, currency)}</Text>
          </View>
          <View style={styles.heroCard}>
            <Text style={styles.heroLabel}>{l.withBalance}</Text>
            <Text style={styles.heroValueBlue}>{data.summary.suppliersWithBalance}</Text>
          </View>
        </View>
          ...
          <View style={styles.table}>
            ...
            {data.suppliers.map((row, i) => (
              <View key={row.id} style={i % 2 === 0 ? styles.tableRow : styles.tableRowAlt} wrap={false}>
                ...
                <Text style={[styles.cellText, styles.colBalance, styles.alignRight, { color: row.balance > 0 ? "#dc2626" : "#0f172a" }]}>{formatAmount(row.balance, currency)}</Text>
                <Text style={[styles.cellText, styles.colInvoices, styles.alignRight]}>{row.invoiceCount}</Text>
                <Text style={[styles.cellText, styles.colStatus, { color: row.isActive ? "#15803d" : "#64748b" }]}>{row.isActive ? l.activeLabel : l.inactive}</Text>
              </View>
            ))}
            <View style={styles.tableTotalsRow} wrap={false}>
              <Text style={[styles.cellTextBold, { width: "58%", paddingRight: 4 }]}>{l.totals} ({data.summary.totalSuppliers})</Text>
              <Text style={[styles.cellTextBold, styles.colBalance, styles.alignRight]}>{formatAmount(data.summary.totalPayable, currency)}</Text>
