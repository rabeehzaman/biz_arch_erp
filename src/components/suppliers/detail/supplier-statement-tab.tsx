...
export function SupplierStatementTab({ supplierId }: { supplierId: string }) {
  const { t } = useLanguage();

  const typeLabels: Record<string, string> = {
    OPENING_BALANCE: t("statement.typeOpeningBalance"),
    PURCHASE_INVOICE: t("statement.typePurchaseInvoice"),
    PAYMENT: t("statement.typePayment"),
    DEBIT_NOTE: t("statement.typeDebitNote"),
  ...
  const fetchStatement = async () => {
    setIsLoading(true);
    try {
      let url = `/api/suppliers/${supplierId}/statement`;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      let url = `/api/suppliers/${supplierId}/statement/pdf`;
      const params = new URLSearchParams();
      if (fromDate) params.append("from", fromDate);
      if (toDate) params.append("to", toDate);
      if (params.toString()) url += `?${params.toString()}`;
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-500">{t("statement.openingBalance")}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-500">{t("statement.totalPayable")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-lg font-bold text-red-600 sm:text-2xl">{formatCurrency(statement.totalDebits)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-500">{t("statement.totalPaid")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-lg font-bold text-green-600 sm:text-2xl">{formatCurrency(statement.totalCredits)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-500">{t("statement.closingBalance")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className={`text-lg font-bold sm:text-2xl ${statement.closingBalance >= 0 ? "text-green-600" : "text-red-600"}`}>
              {formatCurrency(statement.closingBalance)}
              <span className="text-sm ml-1">{statement.closingBalance >= 0 ? "Dr" : "Cr"}</span>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          {statement.transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-sm text-slate-500">
                {fromDate || toDate ? t("statement.adjustDateFilter") : t("statement.noSupplierTransactions")}
              </p>
            </div>
          ) : (
            <>
                  ...
                  <div key={txn.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                      <div className="rounded-lg bg-rose-50 px-3 py-2">
                        <p className="text-xs text-slate-500">{t("statement.payable")}</p>
                        <p className="font-semibold text-red-600">{txn.debit > 0 ? formatCurrency(txn.debit) : "-"}</p>
                      </div>
                      <div className="rounded-lg bg-emerald-50 px-3 py-2">
                        <p className="text-xs text-slate-500">{t("statement.paid")}</p>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">{t("statement.payable")}</TableHead>
                      <TableHead className="text-right">{t("statement.paid")}</TableHead>
