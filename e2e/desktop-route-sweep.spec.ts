  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await page.evaluate(() => {
        ...
        const fixedNavCount = Array.from(document.querySelectorAll("nav")).filter((element) => {
        ...
        return {
          ...
          fixedNavCount,
          mainOpacity: mainStyle?.opacity ?? null,
...
test("admin desktop route sweep", async ({ page, baseURL }) => {
  ...
  for (const route of routes) {
    console.log(`[desktop-route-sweep][admin] ${route}`);
    ...
    if (audit.fixedNavCount > 0) {
      issues.push(`${route}: fixed nav count ${audit.fixedNavCount}`);
    }

    if (audit.mainOpacity && audit.mainOpacity !== "1") {
      issues.push(`${route}: main opacity is ${audit.mainOpacity}`);
    }

    if (audit.mainTransform && audit.mainTransform !== "none") {
      issues.push(`${route}: main transform is ${audit.mainTransform}`);
    }
  }
...
test("superadmin desktop route sweep", async ({ browser, baseURL }) => {
  ...
  try {
    ...
    for (const route of routes) {
      console.log(`[desktop-route-sweep][superadmin] ${route}`);
